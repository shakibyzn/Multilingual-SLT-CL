...
def run(frame_roots, annotations, outroot, inp_channels='rgb'):
    ...
    for root, annot in zip(frame_roots, annotations):
        ...
        for pth in tqdm(glob.glob(os.path.join(root, "*"))):
            # print(pth)
            # print(os.path.basename(pth).strip()[:-4])
            # print(df[df["SENTENCE_NAME"] == os.path.basename(pth).strip()[:-4]])
            if pth.endswith('.mp4'):
                frames = load_rgb_frames_from_video(pth, logger, desired_channel_order=inp_channels)
...
if __name__ == '__main__':
    ...
    frame_roots = [
        'feature_ext/how2sign/dev',
        'feature_ext/how2sign/test',
        'feature_ext/how2sign/train'
    ]
    ...
    out = 'feature_ext/data/s3d-features/how2sign'
