...
def run(weight, frame_roots, annotations, outroot, inp_channels='rgb'):
    ...
    for set_type in ['dev', 'test', 'train']:
        logging.info(f"Extracting features for {set_type} set")
        out_results = []
        for item in tqdm(frame_roots['info']):
            ...
            if item_split != None and item_split == set_type:
                ...
                video_name = os.path.join('CSL-Daily/sentence/frames_512x512', name)
        ...
        save_features_to_file(out_results, os.path.join(outroot, set_type + '_i3d.npz'))
...
if __name__ == '__main__':
    ...
    with open('CSL-Daily/sentence_label/csl2020ct_v2.pkl', 'rb') as f: 
        frame_roots = pickle.load(f) 
    
    annotations = pd.read_csv('CSL-Daily/sentence_label/split_1.txt', sep='|')

    out = 'data/i3d-features/csl-daily'
