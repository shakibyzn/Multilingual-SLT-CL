...
def validate_on_data(
    ...
    with torch.no_grad():
        ...
        total_num_seqs = 0
        total_num_samples = 0
        total_num_samples_valid = 0
        for valid_batch in iter(valid_iter):
            batch = Batch(
                lang=languages,
                sign_lang_vocab=model.sign_lang_vocab,
                is_train=False,
            ...
            if languages != 'all':
                batch_len = len(batch.sequence)
                total_num_samples += batch_len
                # sort_reverse_index = batch.sort_by_sgn_lengths()
                new_data_indices.extend(torch.add(batch.indices, total_num_samples_valid).tolist())
                total_num_samples_valid += len(valid_batch)
        ...
        if do_translation:
            if languages != 'all':
                assert len(all_txt_outputs) == total_num_samples # check-it
            else:
            ...
            if languages != 'all':
                txt_ref = ["".join(t).replace("▁", " ") for i, t in enumerate(data.txt) if i in new_data_indices]
            else:
            ...
            if languages != 'all':
                langs = [i[0] + "-" + j[0] for i, j in zip(data.sign_lang, data.lang) for lg in languages if lg in i[0]]
            else:
                langs = [i[0] + "-" + j[0] for i, j in zip(data.sign_lang, data.lang)]
                
            if languages != 'all':
                sign_lang_kind = [
                    i + "-" + SIGN_LANG_MAPPER[i]
                    for i in model.sign_lang_vocab.itos for lg in languages if lg in i
                ]
            else:
                sign_lang_kind = [
                    i + "-" + SIGN_LANG_MAPPER[i]
    ...
    if languages != 'all':
        results["valid_sequences"] = [t for i, t in enumerate(data.sequence) if i in new_data_indices]
    else:
        results["valid_sequences"] = [t for t in data.sequence]
...
def test(
    ...
    if output_path is None:
        output_path = os.path.join(cfg['training']['model_dir'], 'best.IT')
    ...
    kind_of_langs = cfg["training"]["adapter_languages"] if cfg["training"].get("use_adapter", False) else cfg["training"]["languages"]
    model = build_model(
        cfg=cfg["model"],
        ...
        languages=kind_of_langs,
    )

    if cfg["training"].get("use_adapter", False):       
        model_dir = cfg["training"]["model_dir"] 
        ...
        # load adapters
        load_adapters(model, model_dir, cfg["training"]["adapter_languages"])
    ...
    def _write_to_file(file_path: str, sequence_ids: List[str], references: List[str], hypotheses: List[str]):
        with open(file_path, mode="w", encoding="utf-8") as out_file:
            for seq, ref, hyp in zip(sequence_ids, references, hypotheses):
                out_file.write(seq + "|" + ref + "|" + hyp + "\n")
    ...
    if output_path is not None:
        ...
        if do_translation:
            ...
            _write_to_file(
                ...
                [s for s in dev_data.sequence],
                dev_best_translation_result['txt_ref'],
