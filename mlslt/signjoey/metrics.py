...
bleurt_scorer = score.BleurtScorer("data/bleurt-ckpt/BLEURT-20")
...
def bleurt(references, hypotheses):
    """
    BLEURT score from Google
    :param hypotheses: list of hypotheses (strings)
    :param references: list of references (strings)
    :return:
    """
    scores = bleurt_scorer.score(references=references, candidates=hypotheses, batch_size=128)
    scores = 0 if len(scores) == 0 else sum(scores) / len(scores)
    return scores
