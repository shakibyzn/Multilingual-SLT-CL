...
from signjoey import ewc
from signjoey import er as exp_replay

from torch import Tensor
from torch.utils.tensorboard import SummaryWriter
from torchtext.data import Dataset
from typing import List, Dict, Union
from signjoey import SIGN_LANG_MAPPER
...
def freeze_base_parameters(model):
    ...
    # Unfreeze the parameters of the adapter modules and the attention weights
    for name, param in model.named_parameters():
        if any(substring in name for substring in relevant_substrings):
...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        # adapters
        self.use_adapter = train_config.get("use_adapter", False)
        self.languages = train_config.get("languages", None)
        
        # continual learning
        self.cont_learn_alg_name = train_config.get("cont_learn_alg", None)
        if self.cont_learn_alg_name == 'ewc':
            self.ewc_lambda = train_config.get("ewc_lambda", 20)
        
        elif self.cont_learn_alg_name == 'er':
            self.memory_size = train_config.get("memory_size", 100)
        ...
        if "load_model" in train_config.keys():
            ...
            use_adapter = train_config.get("use_adapter", False)
            freeze_base = train_config.get("freeze_base", False)
    ...
    def _save_checkpoint(self) -> None:
        ...
        if self.use_adapter is True:
            save_adapters(self.model, self.model_dir, self.languages)
    ...
    def init_from_checkpoint(
        ...
        :param use_adapter: use adapter modules from the checkpoint
        :param freeze_base: freeze base model parameters
        """
        ...
        if not use_adapter:
            ...
            # remove adapter modules from pretrained model
            # for name, param in new_model_dict.items():
            #     if 'adapter_modules' in name:
            #         new_model_dict[name] = torch.nn.Parameter(torch.zeros_like(param))
            ...
            #     for name, param in model.named_parameters():
            #         if 'adapter_modules' in name:
            #             print(f"Gradient for {name}: {param.grad}, param: {param}")
            
            # verify_gradients(self.model)
            
            if freeze_base:
                freeze_base_parameters(self.model)
                self.logger.info("Using Adapters; Freezing base model parameters.")
            else:
                self.logger.info("Using Adapters; Not freezing base model parameters.")
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset, lang: Union[List[str], str]) -> None:
        """
        ...
        """
        self.logger.info("Freezing language specific parameters.")
        freeze_langs_spec_parameters(self.model, lang, self.logger)
        ...
        if self.cont_learn_alg_name == "ewc":
            self.cont_learn_alg = ewc.EWC(self, self.model, train_data)
        
        elif self.cont_learn_alg_name == "er":
            if len(lang) > 1:
                self.cont_learn_alg = exp_replay.ExperienceReplay(self, self.model, train_data, self.languages[:-1], self.memory_size)
                
            er_lang = [self.languages[-1]]
        
        self.logger.info(f"Lang vocab: {self.model.lang_vocab}")
        self.logger.info(f"Sign lang vocab: {self.model.sign_lang_vocab}")
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch_idx, batch in enumerate(iter(train_iter)):
                ...
                batch = Batch(
                    lang=er_lang if self.cont_learn_alg_name == "er" else lang,
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    self._log_examples(
                        sequences=[s for s in val_res['valid_sequences']],
                    ...
                    valid_seq = [s for s in val_res['valid_sequences']]
    ...
    def _train_batch(self, batch_idx: int, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        """
        ...
        # penalty if continual learning is enabled
        if self.do_translation:
            if self.cont_learn_alg_name == "ewc":
                translation_loss += self.ewc_lambda * self.cont_learn_alg.penalty(self.model)
            
            elif self.cont_learn_alg_name == "er":
                # skip if there is only one language and replay every 10 batches
                if len(self.languages) > 1 and batch_idx % 10 == 0:
                    self.logger.info(f"Replaying from memory for batch {batch_idx}")
                    translation_loss += self.cont_learn_alg.replay(self.batch_size)
...
def train(cfg_file: str) -> None:
    ...
    model = build_model(
        ...
        use_adapter=cfg["training"].get("use_adapter", False),
        languages=cfg["training"]["languages"],
    )
    ...
    # print trainable parameters
    # print_trainable_parameters(model, logger=trainer.logger)
    ...
    # languages
    sign_langs = cfg["training"]["languages"]
    if set(cfg["training"]["languages"]) == set(SIGN_LANG_MAPPER.keys()):
        sign_langs = "all"
    
    trainer.logger.info(f"Sign languages: {sign_langs}")
    # train the model
    trainer.train_and_validate(train_data=train_data, valid_data=dev_data, lang=sign_langs)
